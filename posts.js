"use strict";
  ...
  function loadAlbums() {
  	d3.select("#albums").append("h3").text("Who likes my posts?");
    d3.select("#albums")
      .append("a")
      .attr("href", "#")
      .on("click", function(d) {
	...
	var album =d3.select("#albumsData").selectAll(".album")
  		...
  		album
  			...
  			.attr("style", "padding-right: 5px")
  				.attr("src", function(d) { if (d.icon) {return d.icon;} else { return "undefined.gif"} } )
  			// .attr("alt", function(d) {
  			// 		return d.name + " with " + d.count + " photos" })
  			.attr("title", function(d) {
  					return d.type + " with " + (d.likes&&d.likes.data? d.likes.data.length:0) + " likes" + " created at " + d.created_time});
